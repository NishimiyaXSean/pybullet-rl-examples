...
def manual_control():
    ...
    env.frame_skip = 1           # 关掉跳帧，把键盘响应延迟降到极低
    env.EPISODE_LEN_SEC = 60     # 把一局的时间拉长到 60 秒
    obs, info = env.reset()
    ...
    while True:
        ...
        # 退出与重置
        if 27 in keys and keys[27] & p.KEY_WAS_TRIGGERED: break # ESC 退出
        ...
        # BFM 11维机动键盘映射
        if ord('2') in keys and keys[ord('2')] & p.KEY_IS_DOWN: action = 1       # 加速
        elif ord('3') in keys and keys[ord('3')] & p.KEY_IS_DOWN: action = 2     # 减速
        elif ord('w') in keys and keys[ord('w')] & p.KEY_IS_DOWN: action = 3     # 跃升
        elif ord('s') in keys and keys[ord('s')] & p.KEY_IS_DOWN: action = 4     # 俯冲
        elif ord('a') in keys and keys[ord('a')] & p.KEY_IS_DOWN: action = 10    # 左转
        elif ord('d') in keys and keys[ord('d')] & p.KEY_IS_DOWN: action = 9     # 右转
        elif ord('q') in keys and keys[ord('q')] & p.KEY_IS_DOWN: action = 5     # 左转跃升
        elif ord('e') in keys and keys[ord('e')] & p.KEY_IS_DOWN: action = 7     # 右转跃升
        elif ord('z') in keys and keys[ord('z')] & p.KEY_IS_DOWN: action = 8     # 左转俯冲
        elif ord('c') in keys and keys[ord('c')] & p.KEY_IS_DOWN: action = 6     # 右转俯冲
