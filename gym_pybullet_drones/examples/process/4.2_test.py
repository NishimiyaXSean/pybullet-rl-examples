...
class DronePIDEnv(CtrlAviary):
    def __init__(self, gui=False):
        ...
        self.action_durations_sec = {
            0:  0.16,  # 匀速直飞（默认状态，给个较短的时间以保持灵活性，约10帧）
            1:  0.4,   # 加速
            2:  0.5,   # 减速
            3:  0.6,   # 跃升
            4:  0.6,   # 俯冲
    ...
    def step(self, action):
        ...
        # 如果当前高度低于 1.5 米，且 AI 试图执行俯冲动作 (n_n < 0)
        # 强制将机动拉平，防止前期盲目探索导致秒死
        if state[2] < 1.5 and n_n < 0:
            n_n = 1.0   # 强行改为平飞过载
            mu = 0.0    # 强行回正滚转角
        ...
        for _ in range(repeat):   
            ...
            alive_bonus = 0.8   # 每帧存活奖励
            height_penalty = max(0.0, 1.5 - cur_pos[2]) * 2.0   # 低于 1.5m 逐渐惩罚
            ...
            # === 总奖励 ===
            reward = reward_tracking * 1.5 + reward_distance + reward_action *0.5 + alive_bonus - height_penalty + reward_terminal
            total_reward += reward 
...
def train():
    ...
    try:
        model.learn(total_timesteps=600_000, reset_num_timesteps=False,tb_log_name="run_4.2_BFM") 
