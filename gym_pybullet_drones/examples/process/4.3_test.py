...
class DronePIDEnv(CtrlAviary):
    ...
    def step(self, action):
        ...
        for _ in range(repeat):   
            ...
            # 引导项：基于势能的靠近奖励 (Progress Reward) 【新增】
            ...
            penalty_time = -0.5 * dt
            penalty_action = -0.05 * (abs(n_x) + abs(n_n - 1.0) + abs(mu)) * dt
            ...
            # === 每帧的总奖励全部为负，直至截击成功 ===
            reward = penalty_tracking + penalty_distance + penalty_time + penalty_action + penalty_height + reward_terminal
            total_reward += reward 
...
MODEL_PATH = "drone_vision_v7"
VEC_NORM_PATH = "vec_normalize_vision_v7.pkl"
...
def train():
    ...
    try:
        model.learn(total_timesteps=600_000, reset_num_timesteps=False,tb_log_name="run_4.3_BFM") 
