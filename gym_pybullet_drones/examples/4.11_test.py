...
class DronePIDEnv(CtrlAviary):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.target_mode = np.random.choice([0, 1, 2, 3])  
        ...
        if self.target_mode in [0, 1]:
            ...
            self.target_omega = np.random.choice([-0.4, 0.4])   # 随机顺时针或逆时针
    ...
    def step(self, action):
        ...
        for _ in range(repeat):   
            ...
            # 当距离小于杀伤半径(例如 0.8m)，且距离开始变大(frame_delta_dist > 0)时，说明刚刚掠过目标！
            # 此时的 prev_dist 就是本次攻击的“脱靶量 (Miss Distance)”
            elif dist < 0.8 and frame_delta_dist > 0:
                miss_distance = self.prev_dist
                
                # 根据脱靶量计算梯度得分：
                # 脱靶量 0.15m 附近 -> 得分接近 300
                # 脱靶量 0.80m 附近 -> 得分接近 50
                # 公式：基础分50 + 250 * (1 - (脱靶量 - 0.15) / 杀伤区间)
                score_ratio = 1.0 - ((miss_distance - 0.15) / (0.8 - 0.15))
        ...
        if terminated or truncated:
            # 只有拿到了 300 分的终极奖励，才算成功 (True)，超时或坠毁都是失败 (False)
...
def train():
    ...
    try:
        model.learn(total_timesteps=200_000, reset_num_timesteps=False,tb_log_name="run_4.11_BFM") 
