...
class DronePIDEnv(CtrlAviary):
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.target_mode in[0, 1]:
            self.target_v[self.target_mode] = np.random.choice([-0.4, 0.4])
            self.target_pos = self.target_anchor.copy() 
        elif self.target_mode == 2:
            self.target_v[2] = np.random.choice([-0.3, 0.3])
            self.target_pos = self.target_anchor.copy()
        elif self.target_mode == 3:
            self.target_angle = np.random.uniform(0, 2 * np.pi) 
            self.target_omega = np.random.choice([-0.4, 0.4])   
    ...
    def step(self, action):
        ...
        # 解析 3 维动作
        v_forward = ((action[0] + 1.0) / 2.0) * 3.5  # X轴：前进 (最大 3.5m/s)
        v_up = action[1] * 2.5       # Z轴：上升/下降 (最大 2.5m/s)
        yaw_rate = action[2] * 2.0   # 转向：左转/右转 (最大 2.0 rad/s)
        
        # 强制侧向速度为 0！(Y轴 = 0.0)
        target_vel_local = np.array([v_forward, 0.0, v_up]) 
        ...
        reward_progress = progress * 200.0 
        
        # 模块 2：雷达火控锁定奖励 (Active Radar Seeker)
        # 只要有射线打中红球，每根射线给 0.005 分。
        reward_radar = self.current_lidar_hits * 0.005
...
MODEL_PATH = "drone_lidar_v2"
VEC_NORM_PATH = "vec_normalize_lidar_v2.pkl"
...
def test():
    ...
    print("  按键 [2] : 上帝俯视全景视角 (自上而下)")
    print("  按键 [3] : 动态迎击视角 (以红球为中心，动态变焦)")
    ...
    center_line_id = -1 # ✨ 新增：用于记录中轴线的 ID
    camera_mode = 1
    camera_mode = 4 
    episode_count = 0
    ...
    while episode_count < 20: 
        ...
        if ord('1') in keys and keys[ord('1')] & p.KEY_WAS_TRIGGERED: camera_mode = 1
        if ord('2') in keys and keys[ord('2')] & p.KEY_WAS_TRIGGERED: camera_mode = 2
        if ord('3') in keys and keys[ord('3')] & p.KEY_WAS_TRIGGERED: camera_mode = 3
        if ord('4') in keys and keys[ord('4')] & p.KEY_WAS_TRIGGERED: camera_mode = 4
        if ord('5') in keys and keys[ord('5')] & p.KEY_WAS_TRIGGERED: camera_mode = 5

        action, _ = model.predict(obs, deterministic=True)
        obs, reward, done, info = env.step(action)

        cur_pos = base_env._getDroneStateVector(0)[0:3]
        cur_quat = base_env._getDroneStateVector(0)[3:7] # 获取当前姿态
        ...
        if not done[0]:
            ...
            # === 画线 3：雷达中心准星轴线 (纯蓝色，加粗)
            rot_mat = np.array(p.getMatrixFromQuaternion(cur_quat)).reshape(3, 3)
            # 获取雷达正中心的射线 (7x7=49 根射线中的第 24 根)
            center_ray_local = base_env.ray_ends_local[24]
            center_end_world = cur_pos + np.dot(center_ray_local, rot_mat.T)
            
            if center_line_id == -1:
                center_line_id = p.addUserDebugLine(cur_pos, center_end_world,[0, 0, 1], 4.0, 0, physicsClientId=base_env.CLIENT)
            else:
                p.addUserDebugLine(cur_pos, center_end_world,[0, 0, 1], 4.0, 0, replaceItemUniqueId=center_line_id, physicsClientId=base_env.CLIENT)
        
            if camera_mode == 1:
                cam_pos = cam_pos * 0.95 + cur_pos * 0.05
                smooth_yaw = np.degrees(base_env.target_yaw)
                p.resetDebugVisualizerCamera(2.0, smooth_yaw - 90, -20, cam_pos, physicsClientId=base_env.CLIENT)
            elif camera_mode == 2:
                p.resetDebugVisualizerCamera(8.0, 0, -89.9, [0, 0, 1.0], physicsClientId=base_env.CLIENT)
            elif camera_mode == 3:
                ...
                p.resetDebugVisualizerCamera(4.0, 45, -30, cur_target_pos, physicsClientId=base_env.CLIENT)
            elif camera_mode == 5:
                anchor_z = base_env.target_anchor[2] 
                p.resetDebugVisualizerCamera(10.0, 90, -5, [0, 0, anchor_z], physicsClientId=base_env.CLIENT)
...
def manual_control():
    ...
    while True:
        ...
        if p.B3G_UP_ARROW in keys and keys[p.B3G_UP_ARROW] & p.KEY_IS_DOWN: action[1] = 1.0
        if p.B3G_DOWN_ARROW in keys and keys[p.B3G_DOWN_ARROW] & p.KEY_IS_DOWN: action[1] = -1.0
            
        if ord('h') in keys and keys[ord('h')] & p.KEY_IS_DOWN: action[2] = 1.0 # 视角左转
        if ord('k') in keys and keys[ord('k')] & p.KEY_IS_DOWN: action[2] = -1.0 # 视角右转

        # 步进物理环境
        obs, reward, terminated, truncated, info = env.step(action)
        cur_pos = env._getDroneStateVector(0)[0:3]
        cur_quat = env._getDroneStateVector(0)[3:7]
        
        # 画出雷达中心准星轴线 (纯白色，加粗)
        rot_mat = np.array(p.getMatrixFromQuaternion(cur_quat)).reshape(3, 3)
        center_ray_local = env.ray_ends_local[24]
        center_end_world = cur_pos + np.dot(center_ray_local, rot_mat.T)
        
        if center_line_id == -1:
            center_line_id = p.addUserDebugLine(cur_pos, center_end_world, [1, 1, 1], 4.0, 0, physicsClientId=env.CLIENT)
        else:
            p.addUserDebugLine(cur_pos, center_end_world, [1, 1, 1], 4.0, 0, replaceItemUniqueId=center_line_id, physicsClientId=env.CLIENT)
