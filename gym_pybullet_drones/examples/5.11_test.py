...
class DronePIDEnv(CtrlAviary):
    ...
    def reset(self, seed=None, options=None):
        ...
        # 【新增】：敌机雷达告警接收机 (RWR) 状态
        self.rwr_triggered = False
        self.rwr_trigger_dist = np.random.uniform(2.0, 8.0) #触发距离的领域随机化
    ...
    def _computeObs(self):
        state = self._getDroneStateVector(0)
        pos = state[0:3]
        rpy = state[7:10]   # 自身姿态(Roll, Pitch, Yaw)

        # ==========================================
        # 修复 1：坐标系对齐 (Yaw Frame vs Body Frame)
        # ==========================================
        # 以前你使用的是 self.target_yaw (虚拟目标的朝向)
        # 现在改用 rpy[2] (无人机真实的机头朝向)
        actual_yaw = rpy[2]
        pilot_quat = p.getQuaternionFromEuler([0, 0, actual_yaw])
        ...
        # ==========================================
        # 修复 2：致命缺失 - 角速度 (Angular Velocity)
        # ==========================================
        # 提取角速度
        world_ang_vel = state[13:16]
        # 角速度也是三维向量，同样需要转换到第一人称局部坐标系
        local_ang_vel, _ = p.multiplyTransforms([0,0,0], inv_quat, world_ang_vel, [0,0,0,1])
        ...
        # ==========================================
        # 修复 3：PPO 的死穴 - 物理量级预缩放 (Pre-normalization)
        # ==========================================
        # 设定环境的物理极限值 (可根据你的实际环境微调)
        MAX_DIST = 15.0     # 假设战场最大范围 15m
        MAX_VEL = 5.0       # 假设最大速度 5m/s
        MAX_ANG_VEL = 2 * np.pi # 假设最大角速度 (约360度/秒)

        # 将所有的特征缩放到大致 [-1, 1] 的区间内
        norm_local_rel_pos = local_rel_pos / MAX_DIST
        norm_local_vel = local_vel / MAX_VEL
        norm_rpy = rpy / np.pi                  # 角度从 [-pi, pi] 缩放至 [-1, 1]
        norm_local_ang_vel = local_ang_vel / MAX_ANG_VEL
        norm_z_height = z_height / MAX_DIST     # 高度也是距离，用 MAX_DIST 缩放
        norm_local_virt_pos = local_virt_pos / MAX_DIST
        norm_local_target_v = local_target_v / MAX_VEL
        
        # 拼接 19 维特征数组
        obs_array = np.concatenate([
            norm_local_rel_pos,   # 3维
            norm_local_vel,       # 3维
            norm_rpy,             # 3维
            norm_local_ang_vel,   # 3维 (新加入的角速度)
            [norm_z_height],      # 1维
            norm_local_virt_pos,  # 3维
            norm_local_target_v   # 3维
        ]).astype(np.float32)
    ...
    def step(self, action):
        ...
        for _ in range(repeat):   
            ...
            actual_cur_pos = current_state[0:3]

            # ==========================================
            # 【全新智能博弈】：目标雷达告警与主动规避系统
            # ==========================================
            dist_to_uav = np.linalg.norm(self.target_pos - actual_cur_pos)

            # 1. 触发状态机
            if dist_to_uav < self.rwr_trigger_dist:
                self.rwr_triggered = True

            if self.rwr_triggered:
                # --- 智能空战规避机动 (Evasion Mode) ---
                # 动力1：反向逃跑力 (远离无人机)
                vec_away = self.target_pos - actual_cur_pos
                vec_away_norm = vec_away / (np.linalg.norm(vec_away) + 1e-6)
                
                # 动力2：不规则桶滚机动 (Jinking - 制造瞄准困难)
                t_sec = self.step_counter / self.PYB_FREQ
                jink_vec = np.array([np.cos(t_sec * 4.0), np.sin(t_sec * 4.0), np.cos(t_sec * 2.5)])
                
                # 动力3：战场引力 (防止其飞向太空或飞出 15米 边界)
                vec_center = self.target_anchor - self.target_pos
                vec_center_norm = vec_center / (np.linalg.norm(vec_center) + 1e-6)
                
                # 融合三股战术意图：70%背逃 + 20%蛇皮走位 + 10%防出界
                evade_dir = vec_away_norm * 0.7 + jink_vec * 0.2 + vec_center_norm * 0.1
                evade_dir = evade_dir / (np.linalg.norm(evade_dir) + 1e-6)
                
                target_v_desired = evade_dir * 1.2  # 理想的极限逃窜速度
                
                # 【物理惯性平滑 (Break Turn 模拟)】
                # 0.03 是平滑系数。60Hz下，大约需要 1~1.5 秒完成大角度转弯
                # 这完美模拟了真实战斗机遭遇锁定时的“急转弯脱离”轨迹！
                smooth_factor = 0.03 
                self.target_v = self.target_v * (1.0 - smooth_factor) + target_v_desired * smooth_factor
                self.target_pos += self.target_v * dt
                
                # 敌机低空防撞地保护
                if self.target_pos[2] < 1.0:
                    self.target_pos[2] = 1.0
                    self.target_v[2] = abs(self.target_v[2]) * 0.5
            ...
            # 当距离小于杀伤半径，且距离开始变大(frame_delta_dist > 0)时，说明刚刚掠过目标！
...
MODEL_PATH = "drone_RL_v1"
VEC_NORM_PATH = "vec_normalize_RL_v1.pkl"
...
def train():
    ...
    try:
        model.learn(total_timesteps=300_000, reset_num_timesteps=False,tb_log_name="run_4.23") 
