...
from stable_baselines3.common.vec_env import VecFrameStack # 导入叠帧模块
...
class DronePIDEnv(CtrlAviary):
    def __init__(self, gui=False):
        ...
        self.target_t = 0.0         # 记录当前回合的目标运动时间
        self.target_phase = np.zeros(3) # 简谐运动的初始相位
        self.target_amp = np.zeros(3)   # 简谐运动的振幅
    ...
    def reset(self, seed=None, options=None):
        ...
        # 随机抽取本局的运动模式
        self.target_mode = np.random.randint(0, 6)
        self.target_v = np.zeros(3)
        self.target_t = 0.0 # 重置时间
        ...
        if self.target_mode in [0, 1]:
            ...
            # 新增：Mode 4 三维螺旋 (圆周 + Z轴往复)
            self.target_angle = np.random.uniform(0, 2 * np.pi) 
            self.target_omega = np.random.choice([-0.6, 0.6])   
            self.target_radius = np.random.uniform(1.0, 2.5) 
            self.target_v[2] = np.random.choice([-0.5, 0.5]) # Z轴有初始速度
            self.target_pos = self.target_anchor.copy()
            
        elif self.target_mode == 5:
            # 新增：Mode 5 三维简谐变加速运动 (Sine Wave)
            # 各轴随机分配振幅(0到2米)和角频率(0.5到1.5 rad/s)
            self.target_amp = np.random.uniform(0.5, 2.0, 3) 
            self.target_omega_3d = np.random.uniform(0.5, 1.5, 3)
            self.target_phase = np.random.uniform(0, 2 * np.pi, 3)
            self.target_pos = self.target_anchor.copy()
            
        elif self.target_mode == 6:
            # 新增：Mode 6 抛物线弹跳运动
            self.target_pos = self.target_anchor.copy()
            self.target_pos[2] = 3.0 # 抛物线初始高度给高一点
            # 随机给一个水平初速度 (X和Y方向)
            self.target_v = np.array([
                np.random.uniform(-0.6, 0.6),
                np.random.uniform(-0.6, 0.6),
                np.random.uniform(0, 1.0) # Z轴初速度（稍微上抛或下抛）
            ])
    ...
    def step(self, action):
        ...
        if self.target_mode in [0, 1, 2]:
            ...
            self.target_v[1] =  self.target_radius * self.target_omega * np.cos(self.target_angle)
            # Z走匀速直线
            self.target_pos[2] += self.target_v[2] * dt
            if abs(self.target_pos[2] - self.target_anchor[2]) > 1.5:
                self.target_v[2] *= -1 # 触顶/底反弹

        elif self.target_mode == 5:
            # 三维简谐变加速 (平滑变加速)
            # 公式: pos = anchor + A * sin(wt + phi), v = A * w * cos(wt + phi)
            for i in range(3):
                phase = self.target_omega_3d[i] * self.target_t + self.target_phase[i]
                self.target_pos[i] = self.target_anchor[i] + self.target_amp[i] * np.sin(phase)
                # 求导得到瞬时速度，喂给神经网络
                self.target_v[i] = self.target_amp[i] * self.target_omega_3d[i] * np.cos(phase)

        elif self.target_mode == 6:
            # 抛物线弹跳运动
            g = 2.0 # 模拟重力加速度
            self.target_v[2] -= g * dt # Z轴不断加速下降
            self.target_pos += self.target_v * dt

            # 触地反弹逻辑 (动能损耗设为0.85)
            if self.target_pos[2] < 0.2:
                self.target_pos[2] = 0.2
                self.target_v[2] = abs(self.target_v[2]) * 0.7
            
            # 防止水平飞出边界，撞墙反弹
            for i in [0, 1]:
                if abs(self.target_pos[i] - self.target_anchor[i]) > 3.0:
                    self.target_v[i] *= -1
                    self.target_pos[i] = self.target_anchor[i] + np.sign(self.target_pos[i] - self.target_anchor[i]) * 4.0
        ...
        if new_dist < 0.5:
            reward_proximity = 0.04 * (0.5 - new_dist) 

        # 模块 2：第一人称视觉 (Vision Gaze) - 负责机头朝向
        ...
        if local_xy_dist > 0.2:
            ...
            # 距离 1 米外，严格要求机头对准(权重1.0)；越靠近，权重越小；最后允许盲抓拦截
            vision_weight = np.clip(new_dist / 1.0, 0.0, 1.0)
            reward_vision = -0.1 * vision_weight * (1.0 - cos_yaw_angle)      # 视野偏离得越多，扣分越多！

        # 模块 3：飞行员素养 (Safety & Smoothness) - 负责安全与姿态
        ...
        # === 汇总所有模块 ===
        reward = (reward_progress + reward_proximity +  
...
MODEL_PATH = "drone_vision_v5"
VEC_NORM_PATH = "vec_normalize_vision_v5.pkl"
...
def test():
    ...
    # 剥开三层 Wrapper (VecFrameStack -> VecNormalize -> DummyVecEnv) 拿到真实环境
...
def manual_control():
    ...
    while True:
        ...
        p.addUserDebugLine(prev_pos, cur_pos, [1, 0, 0], 2.5, 1.5, physicsClientId=env.CLIENT)
        prev_pos = cur_pos
