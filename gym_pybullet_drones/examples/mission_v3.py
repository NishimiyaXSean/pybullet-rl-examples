...
class DronePIDEnv(CtrlAviary):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.target_mode = np.random.choice([3, 4, 5])  
        ...
        if self.GUI:
            ...
            p.addUserDebugLine([-10, 0, 0], [10, 0, 0], [1, 0, 0], 4, 0, physicsClientId=self.CLIENT)
            p.addUserDebugText("X-Axis",[10.5, 0, 0], [1, 0, 0], 1.5, physicsClientId=self.CLIENT)
            
            p.addUserDebugLine([0, -10, 0], [0, 10, 0], [0, 1, 0], 4, 0, physicsClientId=self.CLIENT)
            p.addUserDebugText("Y-Axis",[0, 10.5, 0], [0, 1, 0], 1.5, physicsClientId=self.CLIENT)
            
            p.addUserDebugLine([0, 0, 0], [0, 0, 10], [0, 0.5, 1], 4, 0, physicsClientId=self.CLIENT)
            ...
            for val in[-10, -5, 5, 10]:
                # X 轴刻度 (在 X 轴上画小短线，并标字)
                p.addUserDebugLine([val, -0.2, 0],[val, 0.2, 0], [1, 0, 0], 2, 0, physicsClientId=self.CLIENT)
                p.addUserDebugText(f"{val}m", [val, 0.5, 0],[1, 0.5, 0.5], 1.2, physicsClientId=self.CLIENT)
                
                # Y 轴刻度
                p.addUserDebugLine([-0.2, val, 0],[0.2, val, 0], [0, 1, 0], 2, 0, physicsClientId=self.CLIENT)
                p.addUserDebugText(f"{val}m", [0.5, val, 0], [0.5, 1, 0.5], 1.2, physicsClientId=self.CLIENT)
            ...
            for i in range(-10, 11, 2):
                if i != 0: # 避开中心主坐标轴
                    # ==========================================
                    # 【高级视觉优化】：将网格压入“视觉底层”
                    # 1. 颜色改为深空灰/暗铁色 [0.25, 0.25, 0.25]（或者是暗雷达蓝[0.1, 0.2, 0.3]）
                    # 2. 线宽(lineWidth)从 2.5 暴降至 1.0，让其变成极细的辅助线
                    # 3. Z 高度降到绝对底面 0.0，避免与地平线产生视觉冲突
                    # ==========================================
                    grid_color =[0.25, 0.25, 0.25]  # 暗灰色，克制且极具高级感
    ...
    def step(self, action):
        ...
        for _ in range(repeat):   
            ...
            if self.GUI:
                ...
                # ==========================================
                # 精简文字，排版更紧凑
                hud_text = (f"Mode: {self.target_mode} | Dist: {dist_cam:.2f}m\n"
                            f"ATA: {drone_angle:.1f}deg | Vel: {np.linalg.norm(current_state[10:13]):.1f}m/s")
                
                # 让文字始终悬浮在无人机正上方 0.8 米处！(无论怎么运镜都能清晰看到)
                hud_pos =[cur_draw_pos[0], cur_draw_pos[1], cur_draw_pos[2] + 0.8]
                
                # 使用 replaceItemUniqueId 完美解决重叠闪烁问题
                if self.hud_text_id == -1:
                    self.hud_text_id = p.addUserDebugText(hud_text, hud_pos, textColorRGB=[1, 1, 0], textSize=1.2, physicsClientId=self.CLIENT)
                else:
                    self.hud_text_id = p.addUserDebugText(hud_text, hud_pos, textColorRGB=[1, 1, 0], textSize=1.2, replaceItemUniqueId=self.hud_text_id, physicsClientId=self.CLIENT)
...
def plot_mission_trajectory(drone_pos, target_pos, timestamps, episode_id=0):
    ...
    ax1.plot(drone_pos[:,0], drone_pos[:,1], drone_pos[:,2], 
             'b-', linewidth=2.5, label='Drone Trajectory')
    # 红球轨迹
    ax1.plot(target_pos[:,0], target_pos[:,1], target_pos[:,2], 
             'r--', linewidth=2.0, label='Target Trajectory')
    
    # 标注起点和终点
    ax1.scatter(drone_pos[0,0], drone_pos[0,1], drone_pos[0,2], 
                c='blue', s=80, marker='o', label='Start (Drone)')
    ax1.scatter(drone_pos[-1,0], drone_pos[-1,1], drone_pos[-1,2], 
                c='green', s=80, marker='o', label='End (Drone)')
    
    ax1.scatter(target_pos[0,0], target_pos[0,1], target_pos[0,2], 
                c='red', s=80, marker='o', label='Start (Target)')
    ax1.scatter(target_pos[-1,0], target_pos[-1,1], target_pos[-1,2], 
                c='orange', s=80, marker='o', label='End (Target)')
    
    ax1.set_xlabel('X (m)')
    ax1.set_ylabel('Y (m)')
    ax1.set_zlabel('Z (m)')
    ax1.set_title(f'Episode {episode_id} - 3D Pursuit Trajectory')
    ax1.legend(loc='upper left', fontsize=9)
    ax1.grid(True, alpha=0.6)

    # 2. XY 平面投影（最常用查看追击路径）
    ax2 = fig.add_subplot(222)
    ax2.plot(drone_pos[:,0], drone_pos[:,1], 'b-', linewidth=2.5, label='Drone')
    ax2.plot(target_pos[:,0], target_pos[:,1], 'r--', linewidth=2.0, label='Target')
    
    # 起点和终点标注
    ax2.scatter(drone_pos[0,0], drone_pos[0,1], c='blue', s=100, marker='o', label='Start (Drone)')
    ax2.scatter(drone_pos[-1,0], drone_pos[-1,1], c='green', s=100, marker='o', label='End (Drone)')
    ax2.scatter(target_pos[0,0], target_pos[0,1], c='red', s=100, marker='o', label='Start (Target)')
    ax2.scatter(target_pos[-1,0], target_pos[-1,1], c='orange', s=100, marker='o', label='End (Target)')
    ...
    ax3.plot(timestamps, dist, 'g-', linewidth=2.5, label='Distance to Target')
    ax3.set_xlabel('Time (s)')
    ax3.set_ylabel('Distance (m)')
    ax3.set_title(f'Distance over Time (Total: {timestamps[-1]:.2f}s)')
    ax3.grid(True, alpha=0.7)
    
    # 标记杀伤半径
    ax3.axhline(y=0.4, color='r', linestyle='--', alpha=0.7, label='Miss Radius (0.4m)')
    if np.min(dist) < 0.15:
        ax3.axhline(y=0.15, color='darkred', linestyle=':', alpha=0.8, label='Hit Radius (0.15m)')
    ax3.legend()

    # 4. 高度对比曲线
    ax4 = fig.add_subplot(224)
    ax4.plot(timestamps, drone_pos[:,2], 'b-', linewidth=2, label='Drone Height')
    ax4.plot(timestamps, target_pos[:,2], 'r--', linewidth=2, label='Target Height')
    ax4.set_xlabel('Time (s)')
    ax4.set_ylabel('Z Altitude (m)')
    ax4.set_title('Height Comparison')
    ...
    print(f"   时长: {timestamps[-1]:.2f}s | 最小距离: {np.min(dist):.3f}m | 最终距离: {dist[-1]:.3f}m")
