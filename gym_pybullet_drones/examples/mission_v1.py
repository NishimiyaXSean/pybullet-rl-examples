...
class DronePIDEnv(CtrlAviary):
    ...
    def step(self, action):
        ...
        # 当距离小于 0.5 米时，距离越近，给的持续诱惑分越高（最高给 1.0 分/步）
        if new_dist < 0.5:
            reward += 0.1 / (new_dist + 0.1)  
...
MODEL_PATH = "drone_vision_v2"
VEC_NORM_PATH = "vec_normalize_vision_v2.pkl"
...
def test():
    ...
    camera_mode = 2 # 默认 1: 追尾视角, 2: 目标固定广角视角
    episode_count = 0
